...
export function CanvasViewport({ products, branchId, zone, dryZoneMarker, onDryZoneMarkerChange, dryTopZoneMarkers, onDryTopZoneMarkersChange, onProductsChange, onProductsDeleted, onProductsRestored, onRequestAdd, onRegisterCenterPosition, focusProductId, menuOpen }: { products: CanvasProduct[]; branchId: number; zone: string; dryZoneMarker: ZoneMarkerLayout | null; onDryZoneMarkerChange: (layout: ZoneMarkerLayout) => void; dryTopZoneMarkers: Record<DryTopZoneMarkerLabel, ZoneMarkerLayout> | null; onDryTopZoneMarkersChange: (layouts: Record<DryTopZoneMarkerLabel, ZoneMarkerLayout>) => void; onProductsChange: (products: CanvasProduct[]) => void; onProductsDeleted?: (products: CanvasProduct[]) => void; onProductsRestored?: (products: CanvasProduct[]) => void; onRequestAdd: () => void; onRegisterCenterPosition?: (getter: (() => Point) | null) => void; focusProductId?: number | null; menuOpen: boolean }) {
  ...
  const arrange = useCallback((kind: "vertical" | "horizontal" | "grid") => {
    ...
    const next = allProducts.map((item) => {
      ...
      const x = kind === "vertical" ? first.x : first.x + (kind === "horizontal" ? index : index % columns) * (PRODUCT_CHIP_WIDTH + 1);
      const y = kind === "horizontal" ? first.y : first.y + (kind === "vertical" ? index : Math.floor(index / columns)) * (PRODUCT_CHIP_HEIGHT + 1);
      return { ...item, x, y };
    });
